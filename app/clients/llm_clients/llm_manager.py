...
def get_llm_client(provider) -> BaseLLMClient:
    """
    Factory: return the appropriate LLM client for a given LLMProvider.
    Add new providers here — callers never need to change.
    """
    # Import here to avoid circular imports
    from app.schemas.chat_schema import LLMProvider
    if provider == LLMProvider.COHERE:
        return get_summarization_client()
